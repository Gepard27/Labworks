...
const unsigned char CSV_FILE_ERR = 1;
...
int main(int argc, char* argv[]){
    ...
    for (int i = 0; i < args.iteration_; i++) {
        ...
        if (grid.distributeSand() == 1) {
            grid.fillGrid();
            for (short y = grid.minY; y <= grid.maxY; y++) {
                for (short x = grid.minX; x <= grid.maxX; x++) {
                    controller++;
                    if (controller == 1) {
            ...
            unsigned int height_image = grid.maxY - grid.minY + 1;
            unsigned int width_image = grid.maxX - grid.minX + 1;
